...
1. every class in `behaviors` was added to `Services` (or the container behind your custom
   provider) before building the provider;
2. `behaviors` mixes only classes from the mediator's own variant - an asynchronous behavior in a
   synchronous mediator's `behaviors` list, or vice versa, fails the subclass check; and
