...
1. no handler or behavior instance is passed twice — a `Services(*handlers, *behaviors)` call can
   repeat one if a class appears in both lists; and
2. two distinct services really do have distinct types. Two instances of one class configured
   differently cannot both be resolved; give them separate classes if both must be.
