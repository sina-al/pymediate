...
<Callout type="info">
  ...
  construction. A provider whose output type cannot be inferred is rejected at construction
  time — back it with a class or add a concrete return annotation. The
  [dependency injection guide](/docs/guide/dependency-injection) covers nested containers
  and composing the mediator through `providers.Self()`.
</Callout>
