...
`ContainerLike` is a structural protocol matching any object that exposes a `providers` mapping of provider callables. Every dependency-injector container (`DeclarativeContainer`, `DynamicContainer`) satisfies it, so you never implement it yourself — pass your container instance where a `ContainerLike` is expected.
