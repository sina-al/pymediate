...
A stream handler was parameterized on a type that doesn't inherit from `StreamRequest[ChunkType]` (or on a bare `StreamRequest` with no declared chunk type). Because `StreamRequestHandler`'s type parameter is bound to `StreamRequest`, mypy and basedpyright flag this statically too — the runtime error is the backstop for unchecked code.
