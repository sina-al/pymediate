...
The handler's `__call__` has the wrong shape: missing type annotations, wrong parameter count, a parameter annotation that isn't *exactly* the request class declared in `Handler[RequestT]` (a base class or union is rejected — see [the exact-annotation contract](/docs/guide/handlers#the-exact-annotation-contract)), or sync/async mismatch (an `async def __call__` on a sync `Handler`, or vice versa).
