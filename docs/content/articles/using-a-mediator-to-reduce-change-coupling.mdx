...
{/* pmd-note: Fictional Shop application: imports flask and the app's own db/models. */}
...
{/* pmd-note: Fictional Shop application: imports eight of the app's own modules. */}
...
{/* pmd-note: Fictional Shop application: imports csv_export, db, and files. */}
...
{/* pmd-note: Continues the Flask app object defined in the excerpt above. */}
...
{/* pmd-note: Annotations name the fictional export module's dataclasses. */}
...
{/* pmd-note: Startup wiring over the application's own adapters and clients. */}
...
{/* pmd-note: Startup wiring over the application's own operations and dependencies. */}
...
{/* pmd-note: Dispatch table over the application's own operations. */}
...
`Services` holds one instance per exact type, and returns that match when the mediator asks
for one handler instance. If no instance is available, dispatch fails with a configuration error.

The call site preserves the declared response type:

{/* pmd-note: Calls the mediator built by the startup excerpt above. */}
