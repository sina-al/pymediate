...
CQRS (Command Query Responsibility Segregation) separates operations that change state (commands) from operations that read state (queries). PyMediate doesn't enforce CQRS — it's just a naming and design convention layered on top of requests and handlers — but the two map onto each other cleanly. See [Requests and responses: CQRS](../guide/requests-responses.md#cqrs-commands-vs-queries) for the underlying concepts.

This example separates a product catalog into a write side (commands, backed by a primary store) and a read side (queries, backed by a separate read-optimized store), which is the main reason teams reach for CQRS in the first place.
...
Both write and read handlers register on the same `Services` collection and go through the same `Mediator` — CQRS is a split in your domain model, not in PyMediate's dispatch mechanism.
...
In a real system, `ProductReadStore` would typically be backed by a separate replica or a denormalized read model kept in sync via events — this example keeps them in the same process purely to stay runnable as a self-contained snippet.

## Next steps

- [Requests and responses: CQRS](../guide/requests-responses.md#cqrs-commands-vs-queries) - The underlying design pattern.
- [Pipeline behaviors](pipeline-behaviors.md) - Apply different behaviors to commands vs. queries (for example, caching only queries).
