...
PyMediate has no FastAPI-specific integration code — it doesn't need one. FastAPI's `Depends()` is enough to inject a single, application-wide `Mediator` into your route handlers, keeping your endpoints as thin translation layers over domain requests. See [Requests and responses: Framework independence](../guide/requests-responses.md#framework-independence-the-hexagonal-architecture-principle) for why that separation is worth keeping.
...
Catch `HandlerNotFoundError` separately from your domain's own exceptions: it means the application is misconfigured (a handler was never registered), not that the client sent bad input, so it deserves a 500, not a 404. See [Error handling](../guide/error-handling.md) for the rest of PyMediate's exception hierarchy.
...
- [Requests and responses](../guide/requests-responses.md) - Why the route stays a thin adapter.
- [Error handling](../guide/error-handling.md) - PyMediate's exception hierarchy in full.
- [Async/await support](async.md) - Async handlers and mediators.
- [Dependency injection](../guide/dependency-injection.md) - Wiring handlers through a DI container instead of by hand.
