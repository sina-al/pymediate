...
The mypy half of the cross-checker harness (see test_basedpyright.py for the
basedpyright half). Both halves run the same shared snippet corpus and verify:
1. Valid pymediate usage passes mypy type checking
2. Invalid pymediate usage is caught by mypy with the error codes pinned in
   expectations.py
...
from tests.typing.expectations import EXPECTED_MYPY_CODES
...
# tests.typing.snippets.* modules — without this override those suppressions
...
class TestExpectedMypyDiagnostics:
    """Every errors/ snippet must fail with the exact error codes pinned for mypy."""
    ...
    def test_expected_error_codes_reported(self, snippet_file: Path) -> None:
        """Error snippets raise the mypy error codes pinned in expectations.py."""
        exit_code, stdout, stderr = run_mypy_on_file(snippet_file)

        assert exit_code == 1, (
            f"Expected {snippet_file.name} to fail mypy.\nstdout:\n{stdout}\nstderr:\n{stderr}"
        )
        missing = {
            code for code in EXPECTED_MYPY_CODES[snippet_file.stem] if f"[{code}]" not in stdout
        }
        assert not missing, (
            f"{snippet_file.name} should raise {sorted(missing)}, but mypy reported:\n{stdout}"
        )
