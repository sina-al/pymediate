...
If losing a task during restart is acceptable, an in-process task runner requires fewer
components. If work must
...
sequenceDiagram
    ...
    App->>Database: commit state and outbox rows
    App-->>Host: response
    Relay->>Database: claim rows with lease tokens
    Relay->>Queue: publish envelope
    Relay->>Database: mark published with matching token
...
because they share transport contracts and registry code, but they have separate
dependency-injection containers and
...
It does not claim a distributed exactly-once transaction across PostgreSQL, Amazon Simple Queue
...
- timezone-aware Coordinated Universal Time (UTC) `occurred_at`;
...
`shop.ports.outbox` pairs the unchanged envelope with a World Wide Web Consortium (W3C) trace
carrier. `OutboxWriter` is used
...
The separation keeps queue client-library concerns out of persistence, and it lets an application use the
...
`published_at` remains a known duplicate window.
