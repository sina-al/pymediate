...
| Container integration | Emulator or database through Testcontainers | Client-library and server behavior that a process-local implementation cannot prove. |
...
Application unit tests do not use the mediator or Dependency Injector. Their `autospec` helpers
wrap `unittest.mock.create_autospec()` for runtime-checkable port protocols and asynchronous units
of work:
...
database.get_order.return_value = Order.place(
    1,
    7,
    (OrderLine("book", 2, 1_500),),
    date(2026, 7, 13),
)
...
SQLite and PostgreSQL tests cover behavior that sequential tests miss:
...
server-side locks, constraints, and transaction behavior against the PostgreSQL adapter.
...
- a repeatedly failing message reached the configured dead-letter threshold.

The complete local journey crosses the real serialization codec even though its queue is
process-local. Version-specific registry tests include an executable version 1/version 2 example so compatibility
...
Both commands exit with status 0 when their checks pass.
...
Set only the corresponding variable when running the PostgreSQL and MinIO, LocalStack Simple
Queue Service, or Service Bus emulator integration independently.
