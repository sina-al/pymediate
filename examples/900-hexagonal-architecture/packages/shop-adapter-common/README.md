...
`shop-adapter-common` provides stateless implementations used by every Shop deployment.
...
`SystemClock` implements the clock port with the current Coordinated Universal Time (UTC) business
date. Handlers depend on the
...
`FixedExchangeRates` implements the exchange-rate port for the example's British pound (GBP), euro
(EUR), and US dollar (USD) statement
