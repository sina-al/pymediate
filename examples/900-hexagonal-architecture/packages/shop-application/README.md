...
client library, or a concrete infrastructure adapter.
...
`outbox_messages.py` creates a validated integration envelope and captures World Wide Web
Consortium (W3C) trace context beside it. It does not publish. A use case writes the envelope
through its transaction-bound gateway; the
...
`services.logger.StructlogLogger` is the local implementation of the Shop logger port. A
