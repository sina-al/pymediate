...
writing handlers. The application chooses the narrowest block that must commit atomically.
...
component naturally supplies them. It should not expose its client, session, cursor,
client-library message,
or configuration through the protocol.

Ports do not hold credentials, create client-library objects, select provider lifetimes, or choose an
...
graph. Concrete adapter tests cover transaction, concurrency, delivery, and storage behaviour;
