...
async def run_demo() -> None:
    ...
    async with wiring.activate("application", "relay", "consumer"):
        ...
        if customer.customer_id not in storage.exports:
            raise RuntimeError("Expected the order export to be stored")
        ...
        print(f"  background messages: {published} published, {consumed} consumed")
        print(f"  export: memory://exports/{customer.customer_id}.csv")
        print(f"  mail messages: {len(mailer.messages)}")
