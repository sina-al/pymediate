...
`pymediate.sync.Mediator.publish()` sends one event to every handler registered for that
event type. This example publishes `TaskCompleted` to three handlers and shows that they run
...
Each event handler finished before the next started; synchronous publish was sequential.
...
Everything imports from `pymediate.sync` instead of `pymediate`; event handlers declare a
