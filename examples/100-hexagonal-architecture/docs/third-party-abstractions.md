...
This is deliberate. A dependency pointing inward does not require every imported type to have a
...
| OTLP exporter | Encodes telemetry and sends it out of the process | `shop-bindings` |
...
can route, filter, enrich, sample, or fan out telemetry without changing `LoggerBehavior`,
...
Consider a remote pricing capability defined as a gRPC service. gRPC uses Protocol Buffers as an
...
second set of DTOs. It is still a deliberate coupling: the application now accepts the proto's
