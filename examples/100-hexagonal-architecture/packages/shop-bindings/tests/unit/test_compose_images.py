...
def test_runtime_images_use_selected_extras_and_the_unified_configuration_path() -> None:
    ...
    configuration_copy = (
        "COPY --link examples/100-hexagonal-architecture/configuration/${SHOP_PROFILE}.yaml"
    )
    ...
    assert {build["dockerfile"] for build in builds} == {
        "examples/100-hexagonal-architecture/docker/Dockerfile"
    }
...
def test_repository_build_context_exposes_only_required_source_inputs() -> None:
    ...
    assert "!examples/100-hexagonal-architecture/packages/**" in patterns
    assert "!examples/100-hexagonal-architecture/configuration/*.yaml" in patterns
