...
with local, AWS, and Azure infrastructure. That amount of indirection is not a starting requirement
for a PyMediate application. A Python function with container overrides is usually clearer when an
...
- `aws` installs PostgreSQL, S3/SQS, and process-local implementations for services not represented
  by those AWS services;
- `azure` installs PostgreSQL, Blob Storage/Service Bus, and the same process-local support services;
- `observability` installs the OpenTelemetry SDK and OTLP gRPC exporters.
...
- `resource` is a real `providers.Resource` whose implementation returns an asynchronous context
  manager.
...
as asynchronous providers. PyMediate deliberately resolves handler providers synchronously even
...
## A smaller composition root

Data-driven wiring is useful here because infrastructure and host choices are intentionally
orthogonal. A smaller application can keep the same package boundaries and compose the container in
ordinary Python:
