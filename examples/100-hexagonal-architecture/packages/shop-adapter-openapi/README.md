...
Defines `create_app`, the standard ASGI factory. It loads wiring, composes the application
...
spans when a deployment configures an SDK; otherwise the OpenTelemetry API remains a no-op. The
mediator tracing behaviour appears beneath that server span rather than duplicating HTTP metadata.

Uvicorn remains the process runner:

```bash
uv run uvicorn shop.openapi.web:create_app --factory
```

The package does not add a wrapper console script around Uvicorn.

### `shop.openapi.api` and `shop.openapi.routes`

`api` is a small router aggregator. Feature modules under `routes` own orders, customers, invoices,
...
Translates each known structured domain error to an RFC 9457 `application/problem+json` response.
...
Routes return DTOs built field by field from application responses. They do not serialize domain
...
Install one infrastructure profile and the OpenAPI host:

```bash
uv sync --extra default --extra openapi
```

Cloud Compose images install `openapi` with either `aws` or `azure`. They do not contain the worker
...
A gRPC or MCP adapter could sit beside this package. It would translate its own transport models to
the same application requests and would not need to copy route or handler logic.
