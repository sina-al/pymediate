...
Schema creation lives in `sqlite_schema.py`, while `SqliteUnitOfWork` lives in
...
`EphemeralCatalogue` implements the product-catalogue port with a small immutable set of products.
All three profiles use it because this example deliberately leaves a production catalogue service
out of scope. Its state is process-local and it has no resource lifecycle.
...
external effect without assigning a hypothetical payment provider to the domain.
...
cross-process delivery, SDK behavior, redrive configuration, or operational failure modes.
