...
`shop-bindings`. It does not depend on SQS or Service Bus directly; configured queue adapters
implement the messaging ports.

## Two process roles

The `shop-worker` console script exposes two roles that should run as separate processes:
...
Both accept `--once` for tests and operational checks. The distribution is shared, but separate
processes let relay and consumption scale and fail independently.
...
poison message stops returning.
...
Defines two small composition graphs. `RelayContainer` requires only the outbox source and
publisher. `ConsumerContainer` requires the queue, inbox, and application mediator. A relay process
therefore does not construct the application graph, while a consumer deliberately reuses its
mediator.
...
The consumer extracts the W3C carrier stored beside the integration message, creates an
...
The default profile uses `EphemeralMessageBroker`, AWS uses `SqsMessageBroker`, and Azure uses
