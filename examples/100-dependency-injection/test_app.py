"""Tests for the 100-dependency-injection example — the `uv run pytest` entrypoint."""
...
async def test_register_user_returns_created_user(mediator: Mediator) -> None:
...
async def test_get_user_finds_registered_user(mediator: Mediator) -> None:
    registered = await mediator.send(RegisterUser(username="bob"))
...
async def test_get_unknown_user_raises(mediator: Mediator) -> None:
    with pytest.raises(UserNotFoundError):
        await mediator.send(GetUser(user_id=999))


async def test_singleton_repository_is_shared(container: AppContainer, mediator: Mediator) -> None:
    ...
    await mediator.send(RegisterUser(username="carol"))
...
async def test_context_local_singleton_is_shared_within_one_dispatch(
    ...
    await mediator.send(RegisterUser(username="dave"))

    assert container.unit_of_work().entries == ["begin", "registered 'dave'", "commit"]


async def test_context_local_singleton_is_not_shared_across_scopes(
    container: AppContainer, mediator: Mediator
) -> None:
    await mediator.send(RegisterUser(username="erin"))
    first_scope = container.unit_of_work()

    container.unit_of_work.reset()  # simulate moving to the next request

    await mediator.send(RegisterUser(username="frank"))
