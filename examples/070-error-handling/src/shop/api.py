...
decides something else (see ``cli.py``).
"""
...
def create_app(catalog: Catalog | None = None) -> FastAPI:
    ...
    async def on_not_found(request: HTTPRequest, err: ProductNotFoundError) -> JSONResponse:
        return JSONResponse(status_code=404, content={"error": str(err)})

    @app.exception_handler(OutOfStockError)
    async def on_out_of_stock(request: HTTPRequest, err: OutOfStockError) -> JSONResponse:
        return JSONResponse(status_code=409, content={"error": str(err)})

    @app.get("/products/{product_id}")
    async def get_product(product_id: int) -> Product:
        return await mediator.send(GetProduct(product_id=product_id))

    @app.post("/products/{product_id}/orders", status_code=201)
    async def place_order(product_id: int, quantity: int) -> Order:
        return await mediator.send(PlaceOrder(product_id=product_id, quantity=quantity))
