...
class OrderService:
    ...
    async def export_orders(self, customer_id, fmt="csv"): ...

    async def dispatch(self, action: str, payload: dict) -> Any:   # the string front door
...
Auditing moves to one `AuditBehavior` that wraps every request in
...
- Async is the default here. The structural mirror on `pymediate.sync` is
  [005-why-a-mediator-sync](../005-why-a-mediator-sync/) — diff the two to see how small
  the sync delta is.
