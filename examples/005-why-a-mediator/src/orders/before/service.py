...
class OrderService:
    ...
    async def dispatch(self, action: str, payload: dict[str, Any]) -> Any:
        ...
        if action == "cancel_order":
            return await self.cancel_order(payload["order_id"])
        if action == "refund":
            return await self.refund(payload["order_id"], payload["amount"])
        if action == "export_orders":
            return await self.export_orders(payload["customer_id"], payload.get("fmt", "csv"))
