...
real infrastructure (a warehouse, a shipping-rates API, an order database). Each records a
``start``/``done`` pair in a trace so the demo and tests can see the order operations ran
in — here, one after another, because the sync mediator has no concurrency to offer.

This is the synchronous mirror of `examples/050-handler-composition/domain.py` — same
shapes, plain `def` instead of `async def`, no `asyncio.sleep`.
"""
...
class Warehouse:
    ...
    def reserve(self, items: list[str]) -> StockReservation:
...
class ShippingRates:
    ...
    def quote(self, items: list[str]) -> ShippingQuote:
        """Quote shipping for the items."""
