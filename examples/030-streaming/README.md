...
`stream()` returns results incrementally instead of building one complete response first.
This example uses a simulated large language model (LLM) to yield one typed token (`str`)
at a time.
...
- [040-pipeline-behaviors](../040-pipeline-behaviors/) — apply logging, authorization,
  caching, and a transaction boundary around request handlers.
- [030-streaming-sync](../030-streaming-sync/) — the same stream on `pymediate.sync`
  (`Iterator` and plain `for`).
