...
async def test_request_doubles_as_its_own_cache_key(
    mediator: Mediator, cache: dict[GetForecast, Forecast], journal: list[str]
) -> None:
    first = await mediator.send(GetForecast("london"))
    second = await mediator.send(GetForecast("LONDON"))  # equal after normalization
...
async def test_valid_reading_is_stored(mediator: Mediator) -> None:
    ack = await mediator.send(SubmitReading(station_id="st-1", celsius=18.0, api_key="sk-ok"))
