...
This is the synchronous mirror of ``examples/060-messages/handlers.py`` — plain ``def``
handlers instead of ``async def``.
"""
...
class CreateTaskHandler(RequestHandler[CreateTask]):
    ...
    def __call__(self, request: CreateTask) -> Task:
...
class SearchByTagHandler(RequestHandler[SearchByTag]):
    ...
    def __call__(self, request: SearchByTag) -> list[Task]:
...
class RegisterWebhookHandler(RequestHandler[RegisterWebhook]):
    ...
    def __call__(self, request: RegisterWebhook) -> Webhook:
