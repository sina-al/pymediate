...
def create_app(store: DocumentStore | None = None) -> FastAPI:
    ...
    async def on_missing(request: HTTPRequest, err: DocumentNotFoundError) -> JSONResponse:
    ...
    @app.get("/documents")
    async def list_all(http_request: HTTPRequest) -> list[Document]:
        principal = from_http(http_request.headers)
        return await mediator.send(ListAllDocuments(principal=principal))

    @app.put("/documents/{doc_id}")
    async def edit(doc_id: int, body: EditBody, http_request: HTTPRequest) -> Document:
        principal = from_http(http_request.headers)
