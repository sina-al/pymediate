...
def test_default_limiter_enforces_its_quota() -> None:
...
def test_swapping_the_limiter_is_a_constructor_argument() -> None:
...
def test_two_mediators_keep_independent_limiters() -> None:
    ...
    lenient.send(AddTask(title="fine"))
    lenient.send(AddTask(title="also fine"))
