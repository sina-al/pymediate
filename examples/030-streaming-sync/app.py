...
``str``) at a time. The handler's ``__call__`` is a *generator*: it ``yield``\\s chunks
instead of ``return``\\ing a value, and the consumer pulls them with a plain ``for``.
...
``stream``. The async mirror of this example is 030-streaming, built on ``pymediate``.
"""
...
# ---- Stream handler: exactly one per request type; __call__ is a generator ----


class CompletionHandler(StreamRequestHandler[StreamCompletion]):
    """Streams the model's completion token by token.

    ``__call__`` is a *generator* — it ``yield``\\s each ``str`` rather than returning a
    value. Delegating to an existing stream (an LLM client, a DB cursor) stays a one-liner:
    iterate the source, re-yield each chunk.
    """
    ...
    def __call__(self, request: StreamCompletion) -> Iterator[str]:
        yield from self._model.stream(request.prompt)
