...
This example uses `pymediate.sync`. Its tests are plain `def` functions and do not require
`pytest-asyncio`.

It assumes the constructor injection and mediator wiring used in
[100-dependency-injection-sync](../100-dependency-injection-sync/).

## Run

Run these commands from `examples/110-testing-sync`:
...
This test does not need `Services` or `Mediator`.
...
```python
def test_register_user_dispatches_a_welcome_email() -> None:
    sender = FakeSender()
    handler = RegisterUserHandler(sender, UserRepository())

    handler(RegisterUser(username="alice", email="alice@example.com"))
...
- [110-testing](../110-testing/) — use the same test boundaries with the asynchronous API.
- [100-dependency-injection-sync](../100-dependency-injection-sync/) — review constructor
  injection and mediator wiring.
