...
SQLite is a row store: it keeps each product's fields together for inserting a row or updating
one row's stock. The outbox is the important additional table: a
...
This is the transactional-outbox pattern. ``docs/architecture.md`` explains why it avoids the
failure mode created by writing SQLite and DuckDB in the same command.
"""
...
class WriteStore:
    ...
    def adjust_stock(self, product_id: int, delta: int) -> WriteAck:
        ...
        try:
            ...
            if row is None:  # pragma: no cover - the update and read share one transaction
                raise RuntimeError(f"updated product disappeared: {product_id}")
