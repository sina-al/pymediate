...
- **``ReadModel``** — ``find`` / ``search`` / ``inventory_report``. Query handlers depend on
  this protocol, so the type checker rejects attempts to advance the checkpoint or apply a
  batch through that dependency.
- **``ProjectionTarget``** — ``checkpoint`` / ``apply_batch``. The projector depends on this,
  so it never reaches into the query surface.

``ReadStore`` implements both over one DuckDB file. Handlers and the projector receive only
the protocol they need. The composition root retains the concrete store so it can close the
connection and display the checkpoint.
"""
...
class ReadStore:
    ...
    multi-threaded projector would use ``conn.cursor()`` per thread (DuckDB's
