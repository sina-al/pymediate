...
materialises DuckDB in checkpointed batches. The second store's write is therefore outside the
command's failure boundary.

The unit of work and the loop are separate:
...
- ``Projector.drain`` is a plain synchronous method that catches the read model up to the
  ...
  on timing.
- ``ProjectionWorker`` wraps ``drain`` in an ``asyncio`` loop that wakes on a notification (see
  ``handlers.WakeProjector``) and otherwise polls, so a lost notification only adds latency.
"""
...
class ProjectionWorker:
    """Run a ``Projector`` in a background loop: wake on notification, otherwise poll.
    ...
    async def run(self) -> None:
        """Drain, then wait for a notification or poll timeout, until ``stop`` is called."""
        self._running = True
        while self._running:
            ...
            try:
                ...
                pass  # notifications can be lost; the periodic poll still reads the outbox
            self._wake.clear()
