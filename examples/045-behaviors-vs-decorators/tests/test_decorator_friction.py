...
async def test_module_limiter_is_shared_and_eventually_exhausted() -> None:
    handler = decorator.AddTaskHandler(TaskStore())

    await handler(decorator.AddTask(title="one"))
    await handler(decorator.AddTask(title="two"))
    with pytest.raises(RateLimitExceededError):
        await handler(decorator.AddTask(title="three"))  # the module limiter's quota is 2


async def test_swapping_the_limiter_requires_reaching_past_the_class() -> None:
    ...
    try:
        for i in range(5):  # would exceed the real limiter's quota of 2
            await handler(decorator.AddTask(title=f"bulk-{i}"))
    finally:
        decorator._limiter = original  # must restore it, or later tests inherit this one


async def test_two_handler_instances_share_the_same_limiter() -> None:
    ...
    await first(decorator.AddTask(title="one"))
    await second(decorator.AddTask(title="two"))  # counts against the SAME module limiter
    with pytest.raises(RateLimitExceededError):
        await first(decorator.AddTask(title="three"))
