...
This isn't a test — it's a thing you run by hand to *see* why the read side of this example is
DuckDB and not the SQLite write store. It measures **through the application**: both timed
...
def _seed_duckdb(database: Path, products: list[Product]) -> None:
    ...
    Row-by-row ``INSERT`` into a column store is pathologically slow — the wrong way to load an
    OLAP engine. ``COPY`` from a CSV is how you'd actually backfill one.
    """
...
async def main(n: int = 1_000_000, repeats: int = 5) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            for tier in await app.mediator.send(GetInventoryReport()):
                print(
