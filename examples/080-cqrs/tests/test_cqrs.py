...
async def test_command_response_is_minimal(mediator: Mediator) -> None:
...
async def test_query_returns_the_denormalized_view(mediator: Mediator) -> None:
    created = await mediator.send(CreateProduct(name="Keyboard", price=49.99, stock=10))
...
async def test_write_and_read_stores_are_separate_objects(
    mediator: Mediator, write_store: WriteStore, read_store: ReadStore
) -> None:
    await mediator.send(CreateProduct(name="Keyboard", price=49.99, stock=10))
...
async def test_read_model_updates_via_the_event_after_a_stock_adjustment(
    mediator: Mediator,
) -> None:
    created = await mediator.send(CreateProduct(name="Widget", price=9.99, stock=5))

    await mediator.send(AdjustStock(product_id=created.product_id, delta=-5))
    view = await mediator.send(GetProduct(product_id=created.product_id))
...
async def test_search_filters_to_in_stock_only(mediator: Mediator) -> None:
    keyboard = await mediator.send(CreateProduct(name="Keyboard", price=49.99, stock=10))
    out_of_stock = await mediator.send(CreateProduct(name="Discontinued Mouse", price=5, stock=0))
...
async def test_get_missing_product_raises(mediator: Mediator) -> None:
    with pytest.raises(ProductNotFoundError):
        await mediator.send(GetProduct(product_id=999))


async def test_price_tiers(mediator: Mediator) -> None:
    budget = await mediator.send(CreateProduct(name="Cable", price=5, stock=1))
    standard = await mediator.send(CreateProduct(name="Mouse", price=25, stock=1))
    premium = await mediator.send(CreateProduct(name="Monitor", price=250, stock=1))

    budget_view = await mediator.send(GetProduct(product_id=budget.product_id))
    standard_view = await mediator.send(GetProduct(product_id=standard.product_id))
    premium_view = await mediator.send(GetProduct(product_id=premium.product_id))
