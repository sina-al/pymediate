...
def test_command_runs_the_full_stack_in_registration_order(
    mediator: Mediator, trace: list[str]
) -> None:
    mediator.send(AddTask(title="Buy groceries"))
...
def test_query_skips_command_only_behaviors(mediator: Mediator, trace: list[str]) -> None:
    task = mediator.send(AddTask(title="read me"))
    trace.clear()
...
def test_cache_hit_short_circuits_the_handler(
    mediator: Mediator, store: TaskStore, trace: list[str]
) -> None:
    task = mediator.send(AddTask(title="cache me"))
    trace.clear()

    first = mediator.send(GetTask(task_id=task.task_id))
    second = mediator.send(GetTask(task_id=task.task_id))
...
def test_should_apply_skips_uncacheable_query(mediator: Mediator, trace: list[str]) -> None:
    mediator.send(AddTask(title="volatile"))
    trace.clear()

    # ListOpenTasks is uncacheable, so should_apply() drops the caching behavior entirely.
    mediator.send(ListOpenTasks())
    mediator.send(ListOpenTasks())
...
def test_logging_is_universal(mediator: Mediator, trace: list[str]) -> None:
    task = mediator.send(AddTask(title="anything"))
    mediator.send(GetTask(task_id=task.task_id))
    mediator.send(ListOpenTasks())
...
def test_unauthorized_command_is_rejected_before_the_handler(
    ...
    with pytest.raises(AccessDeniedError):
        mediator.send(AddTask(title="denied"))
...
def test_transaction_boundary_records_handler_error(mediator: Mediator, trace: list[str]) -> None:
