...
``TypeRegistry`` — a hand-rolled, name-based registry that has nothing to do with
PyMediate and wasn't built with it in mind, the kind of container many existing apps
already have lying around. Wiring it in touches nothing but construction: every
``mediator.send()`` call site is unchanged.

This is the synchronous mirror of ``120-custom-provider``. The Protocol, the adapter,
and every claim are identical; only the API import and the absence of
``async``/``await`` change.
"""
...
class AddHandler(RequestHandler[Add]):
    ...
    def __call__(self, request: Add) -> int:
...
class MultiplyHandler(RequestHandler[Multiply]):
    ...
    def __call__(self, request: Multiply) -> int:
        result = request.a * request.b
        self._logger.log(f"{request.a} * {request.b} = {result}")
        return result
...
def main() -> None:
    ...
    total = mediator.send(Add(a=2, b=3))
    print(f"2 + 3 = {total}")

    product = mediator.send(Multiply(a=4, b=5))
    print(f"4 * 5 = {product}")
