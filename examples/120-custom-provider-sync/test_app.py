...
import pytest
from pymediate.sync import Mediator, ServiceNotFoundError, Services
...
def test_request_resolves_through_the_custom_provider() -> None:
    mediator = build_mediator()

    result = mediator.send(Add(a=2, b=3))

    assert result == 5


def test_same_call_site_works_with_either_provider() -> None:
    logger = Logger()
    ...
    custom_result = custom_mediator.send(Add(a=10, b=20))
    hand_wired_result = hand_wired_mediator.send(Add(a=10, b=20))
