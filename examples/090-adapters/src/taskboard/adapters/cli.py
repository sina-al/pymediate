...
and ``await mediator.send()`` — the CLI framework runs the event loop. The group
chains, so one invocation runs a whole session against one in-memory store:
``python cli.py add "Buy milk" add "Ship it" complete 1 list``. ``TaskNotFoundError``
maps to click's error convention (message on stderr, exit code 1) via ClickException.
"""
