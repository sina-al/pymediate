...
Compare with `examples/090-adapters-sync/src/taskboard/adapters/fastapi.py` — same
framework, same routes; the only differences are ``async def`` and
``await mediator.send()``. The domain's ``Task`` dataclass doubles as the response
model, and ``TaskNotFoundError`` maps to HTTP 404 via FastAPI's exception_handler hook.
"""
