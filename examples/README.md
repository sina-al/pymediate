...
| 13 | [050-handler-composition](050-handler-composition/) | A handler that needs other operations `send()`s them through the mediator — never holding another handler — and overlaps independent sub-requests with `asyncio.gather`. |
| 14 | [050-handler-composition-sync](050-handler-composition-sync/) | The same composition on `pymediate.sync`, where the sub-requests run sequentially. |
| 15 | [060-messages](060-messages/) | Design requests as value objects: `frozen=True` for an immutable, hashable cache key; `__post_init__` validation that fails at construction; `field(repr=False)` to keep a secret out of logs. |
| 16 | [060-messages-sync](060-messages-sync/) | The same message designs on `pymediate.sync` — identical dataclasses, sync handlers. |
...
compose handlers through the mediator; 15–16 design requests as value objects; 17–18 make
the framework-independence argument; 19 plugs it into a DI container. Async and sync
